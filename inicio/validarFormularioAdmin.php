...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_check = "SELECT usuario FROM useradmin WHERE usuario = ?";
    ...
    $sql = "INSERT INTO useradmin (usuario, password, nombre, apellido) VALUES (?, ?, ?, ?)";
    ...
    if ($stmt) {
        ...
        if ($ejecutar) {
            echo "<script>alert('Registro exitoso. Bienvenido'); window.location='/inventario_saneyCORE/modificarBorrar.php';</script>";
            exit();
        } else {
