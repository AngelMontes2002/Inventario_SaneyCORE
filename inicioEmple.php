...
<head>
    ...
    <style>
        ...
        .product-card {
            ...
            padding: 20px; margin-bottom: 20px; transition: 0.3s; cursor: pointer;
        ...
        .product-icon { font-size: 32px; color: #0d6efd; margin-right: 10px; }
        #carrito {
            position: fixed; top: 80px; right: 0; width: 300px; background: #fff;
            border-left: 2px solid #ccc; padding: 15px; height: 90vh;
            overflow-y: auto; box-shadow: -4px 0 10px rgba(0,0,0,0.1); z-index: 999;
        }
        #carrito h4 { font-weight: bold; margin-bottom: 10px; }
        .carrito-item {
            margin-bottom: 10px; border-bottom: 1px solid #eee; padding-bottom: 8px;
        }
        .header { background: #000; padding: 10px 0; }
        .header .logo h1 { color: white; margin: 0; }
        .navbar-dark .navbar-nav .nav-link {
            color: white !important; font-weight: bold; font-size: 16px; padding: 8px 15px;
        }
...
<div class="container mt-5">
    ...
    <form method="GET" class="mb-4">
        <div class="input-group">
            <input type="text" class="form-control" name="buscar" placeholder="Buscar por nombre o código de producto" value="<?php echo isset($_GET['buscar']) ? htmlspecialchars($_GET['buscar']) : ''; ?>">
    ...
    <div class="row">
        ...
        $consulta = "SELECT * FROM producto WHERE Codigo_pro LIKE '%$buscar%' OR Nombre_pro LIKE '%$buscar%'";
...
<script>
        ...
        for (const codigo in carrito) {
            ...
            lista.innerHTML += `
                <div class="carrito-item">
                    ...
                    Cantidad: <input type="number" min="1" value="${item.cantidad}" onchange="actualizarCantidad('${codigo}', this.value)" style="width: 60px;">
                </div>
            `;
        }
    }
    ...
    function procesarRetiro() {
        ...
        fetch('procesar_retiro.php', {
            ...
            body: JSON.stringify({ id_usuario: ID_USUARIO, productos: productos })
        })
        .then(res => res.json())
        .then(data => {
            if (data.success) {
